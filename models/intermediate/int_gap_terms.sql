...
-- keeps the identity exact and puts the whole difference in the product-mix term.

with totals as (
    select window_id, product_scope, income_band, occupation, sum(carteira_ativa) as occupation_balance
...
products as (
    ...
    from {{ ref('int_cross_section_pooled') }} as pooled
    inner join totals
