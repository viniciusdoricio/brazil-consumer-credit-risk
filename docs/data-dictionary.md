...
| `submodalidade` | Sub-modality, Anexo 3 **[M2]** | 49 (2013-06); 55–56 (2023–2026) **[D]** | **Renamed or split** (dated on the full history: the overdraft and working-capital splits in **2014-02**; `Outros direitos creditórios descontados` **2016-09**; `Cartão de crédito - não migrado` and `Capital de giro com teto rotativo` **2017-05**; `Financiamentos agroindustriais` and `Industrialização` **2017-07**): `Cheque especial e conta garantida` → `Cheque especial` + `Conta Garantida`; `Capital de giro com prazo vencim. igual ou superior 30 d` / `…inferior a 30 d` → `…até 365 dias` / `…superior a 365 dias` / `…com teto rotativo`. New: `Cartão de crédito - não migrado`, `Financiamentos agroindustriais`, `Industrialização`, `Outros direitos creditórios descontados` **[D]** | **Leading/trailing whitespace in 5–8% of rows** (7,487–24,895 rows per month across all 169 months). Trim before grouping **[D]**. Two labels are also broken: `Financiamento habitacional – exceto SFH` is published with a Windows-1252 en dash decoded as the control character U+0096, and the rural `modalidade` label has a doubled space **[D]**. The dbt staging model cleans both (`macros/clean_label.sql`), and the crosswalk to product groups is `seeds/pf_product_groups.csv` |
...
small in R$. From 2017 they are consistent except for one row in December 2024, R$3,088.53 short of
its own identity (section 2.3), which the dbt test on the identity reports as a warning.
...
| **Who has a declared occupation** | SCR held 85 million borrowers in December 2019 (BCB, Estudo Especial 80/2020), while 31.6 million people filed an income-tax return for 2020 and 44.5 million in 2026 (Receita Federal). In 2024, "Outros" held 26% of household balances but 51% of household loans with a reported count, and 56% of the balance in the band up to 1 minimum wage (`src/brazil_consumer_credit_risk/report.py`, facts `outros_*`) **[D]** | **Strong evidence, not confirmed by BCB.** At most about a third of borrowers can carry a declared occupation, and "Outros" has the profile of non-filers: many small loans, concentrated in the lowest bands. Occupation findings describe income-tax filers |
