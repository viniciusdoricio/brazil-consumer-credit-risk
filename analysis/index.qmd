...
- **O consignado se abriu ao setor privado.** O consignado de empregados do setor privado já
  ...
  deles passou de `{python} N.private_payroll_before` em jan/2025 para
...
3. **Acompanhar todo mês a inadimplência de 15 a 90 dias de `{python} N.fastest`, e a do
   consignado do setor privado.** A primeira sobe mais rápido do que a das outras ocupações, com a
   carteira ainda crescendo; a segunda cresce rápido desde a abertura de 2025 e já atrasa mais.
   A inadimplência de 15 a 90 dias é a medida que não depende da regra contábil.
...
- **A ocupação é a declarada no imposto de renda.** Só quem declara tem uma; os demais, ao que
  ...
  atual. As ocupações também foram reclassificadas em vários janeiros. Desde 2017, `{python} N.unexplained_jumps` meses tiveram saltos de mais de
...
- **Parte do que move os números não está nos dados.** Programas de renegociação, tetos de juros,
  ...
  sem que o comportamento dos devedores mude. A seção 5 aponta onde eles coincidem com os resultados, mas o
  SCR.data não permite separar o efeito de cada um.
