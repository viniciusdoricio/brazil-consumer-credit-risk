...
*Translated from the [Portuguese original](../index.html), which is the reference version. [Code on GitHub](https://github.com/viniciusdoricio/brazil-consumer-credit-risk)*
...
- **Payroll-deducted loans (*consignado*)** are repaid straight from a salary or pension. Until 2025
  ...
  March 2025 they have opened up to private-sector workers, a much riskier book
  (@sec-early-arrears).
...
lender and state, under the open ODbL licence. It covers `{python} N.months` months, from
`{python} N.first_month` to `{python} N.latest_month`; in `{python} N.latest_month`, household
credit totalled `{python} N.portfolio`.
...
- **15–90-day delinquency:** the amounts overdue within that range. It is the only measure that
  runs through the 2025 accounting change (@sec-trap).
...
findings describe people who file income tax. "The self-employed" are those who give that
occupation on their return, not Brazil's `{python} N.self_employed_2025` self-employed workers, of
whom only `{python} N.self_employed_with_cnpj` have a business registration.[^pnad] And
`{python} N.retiree_above_one_wage` of the credit to "retirees" sits in bands above 1 minimum wage.
...
or observed roll rates between arrears buckets (@sec-limitations).

# The trap: the January 2025 accounting change {#sec-trap}
...
## Since 2025, early arrears have risen most among the self-employed {#sec-early-arrears}
...
(`{python} N.slowest_change`, with a real balance change of `{python} N.slowest_growth`). The
order doesn't change with the lagged denominator. A rate that rises while credit keeps growing can't be put down
...
3. **Track 15–90-day delinquency every month for `{python} N.fastest`, and for private-sector
   payroll loans.** Among `{python} N.fastest`, it is rising faster than in any other occupation
   while their portfolio still grows; private-sector payroll lending has grown fast since the 2025
   opening, and its early arrears have already risen. 15–90-day delinquency is the measure that
   doesn't depend on the accounting rule.
4. **Don't compare 90-day delinquency before and after January 2025.** A comparison across that
   month mixes an accounting rule with borrower behaviour. Until the effect settles, 15–90-day delinquency is the
   honest comparison.
...
- **Occupation is the one declared on the income-tax return.** Only filers have one; the rest, on
  ...
  current job. Occupations were also reclassified in several Januaries. Since 2017,
  `{python} N.unexplained_jumps` months saw a jump of more than `{python} N.weight_jump` in the
  share of an occupation or income band with no known cause; they are flagged as
  reclassifications, and a test fails if a new one appears.
...
- **Part of what moves the numbers isn't in the data.** Renegotiation programmes, interest caps,
  ...
  without any change in how borrowers behave. @Sec-findings points out where they overlap with the
  results, but SCR.data can't separate their effects.
- **The Banco Central republishes its history.** The analysis uses the file versions listed, with
  dates and hashes, in the repository's [data dictionary][dictionary]. A later version can move
  the numbers slightly; the September 2026 one moved household 90-day delinquency by at most
  `{python} N.republication_shift`.

[^occupation]: Banco Central, [Estudo Especial 80/2020](https://www.bcb.gov.br/conteudo/relatorioinflacao/EstudosEspeciais/EE080_Indicadores_de_endividamento_de_risco_e_perfil_do_tomador_de_credito.pdf) (SCR borrowers, December 2019); Receita Federal, via [Poder360](https://www.poder360.com.br/poder-economia/receita-federal-recebe-445-milhoes-de-declaracoes-do-ir/) (returns filed in 2026). Where occupation comes from is documented in the repository's [data dictionary][dictionary], section 10.4.
...
[dictionary]: https://github.com/viniciusdoricio/brazil-consumer-credit-risk/blob/main/docs/data-dictionary.md
